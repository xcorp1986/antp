...
class DBUtil
{
    /**
     * 获取查询数据集
     * @param \Think\Model|\Think\Model\RelationModel $obj :模型对象
    ...
    public static function queryList(
        ...
        if (!empty($where)) {
            $obj = $obj->where(is_string($where) ? [$where] : $where);
        }
        if (!empty($orderby)) {
    ...
    /**
     * 统计记录总数
     * @param \Think\Model|\Think\Model\RelationModel $obj
     * @param mixed $where
     * @param string|boolean $relation :关联名称,全部关联时为true,没有关联时为false
     * @return number
     */
     ...
     * @param string $field
     * @param mixed $where
     * @param string|bool $relation :关联名称,全部关联时为true,没有关联时为false
     * @return float
     */
    ...
    {
        if ($obj->create($data)) {
            return $obj->where(is_string($where) ? [$where] : $where)->save();
        }
     ...
     * @return array
     */
    public static function buildDistanceField($lng, $lat, $lng_lat_value = [], $field = 'distance')
    {
