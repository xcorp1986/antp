...
class Tool
{
     ...
     * @return mixed
     */
    public static function uploadFile($savePath, $maxSize = 0, $allowExts = [], $thumb = [])
    {
    ...
    public static function getFieldList($data, $field)
    {
        return array_reduce(
            $data,
            create_function('$result=array(),$item', '$result[] = $item["'.$field.'"];return $result;')
        );
    }
     ...
     * @return string
     */
    public static function treeToHtml(
        $data,
        $key = 'id',
        $pKey = 'pid',
        $node = 'text',
    ...
    {
        if (isset($_SERVER['HTTP_X_FORWARDED_FOR']) AND $_SERVER['HTTP_X_FORWARDED_FOR'] AND (!isset($_SERVER['REMOTE_ADDR']) OR preg_match(
                    '/^127\..*/i',
                    trim($_SERVER['REMOTE_ADDR'])
                ) OR preg_match('/^172\.16.*/i', trim($_SERVER['REMOTE_ADDR'])) OR preg_match(
                    '/^192\.168\.*/i',
                    trim($_SERVER['REMOTE_ADDR'])
    ...
    public static function randomUUID($connector = '-')
    {
        ...
        $uuid = str_replace([chr(45)], [$connector], $uuid);
    ...
    {
        $extend = '';
        $secs = ['1' => PATHINFO_DIRNAME, '2' => PATHINFO_BASENAME, '3' => PATHINFO_EXTENSION];
        if (is_file($fileName)) {
            if ($sec == '') {
    ...
    public static function getFileContents($url, $useIncludePath = false, $streamContext = null, $curlTimeOut = 5)
    {
        ...
        } elseif (function_exists('curl_init') && in_array(ini_get('allow_url_fopen'), ['On', 'on', '1'])) {
            $curl = curl_init();
    ...
    public static function callUserMethod($object, $defaultMethod = '', $datas = [])
    {
        if (is_object($object)) {
        ...
        foreach ($methodArray as $method) {
            if (isset($_GET[$method]) || isset($_POST[$method])) {
                call_user_func_array([$current_object, $method], $datas);
                $flag = false;
                break;
            }
        }
        if ($flag) {
            if (method_exists($current_object, $defaultMethod)) {
                call_user_func_array([$current_object, $defaultMethod], $datas);
            }
        }
    }
     ...
     */
    public static function getFileList($filePath, $inc_dir = true, $extends = [])
    {
        $files = [];
        if (is_dir($filePath)) {
     ...
     * @return array:文件数组,形如array('文件路径'=>'文件名')
     */
    public static function getFileLists($filePath, $inc_dir = true, $extends = [])
    {
        $all_files = [];
        if (is_dir($filePath)) {
     ...
     */
    public static function saveArray($file, $array = [])
    {
    ...
    public static function connectByCURL($url, $data, $timeout = 30, $httpHeader = [])
    {
        if (!(function_exists('curl_init') && in_array(ini_get('allow_url_fopen'), ['On', 'on', '1']))) {
            return false;
        }
        $ch = @curl_init();
        if (!$ch) {
